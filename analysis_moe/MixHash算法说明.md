...
| `REORDER_MAX_QUEUE_SIZE` | 8 | 每个队列最大包数（已取消限制） |
...
**仿真配置**:
- tag=2的专家流(8KB) **不使用** 重排序缓冲区
- tag=1的背景大流(8MB) **使用** 重排序缓冲区

**实际Reorder统计数据** (从仿真输出统计):

| 场景 | 背景流数 | Reorder流数 | 实际发生Reorder |
|------|----------|-------------|-----------------|
| MixHash(0) | 0 | 0 | 无 |
| MixHash(64) | 64 | 64 | **无** |
| MixHash(128) | 128 | 128 | **无** |
| MixHash(192) | 192 | 192 | **无** |

**关键发现**:
```
reorder输出文件统计:
  - packets_buffered = 0  (没有包需要缓冲)
  - packets_flushed = 0   (没有包被从缓冲区发出)
  - packets_dropped = 0   (没有包被丢弃)

原因分析:
  1. 网络延迟一致性好（Fat-Tree对称拓扑）
  2. 低负载场景下路径空闲，包按序到达
  3. ecmp_counter的哈希分配没有导致乱序
```

**实际内存占用**:
```
由于没有发生reorder:
  - 每个流创建FlowReorderBuffer结构
  - 但队列(queues)始终为空
  - 内存占用仅为结构开销

各场景实际内存:
  - MixHash(0):   0 流 × 200 bytes = 0 KB
  - MixHash(64):  64 流 × 200 bytes ≈ 13 KB
  - MixHash(128): 128 流 × 200 bytes ≈ 26 KB
  - MixHash(192): 192 流 × 200 bytes ≈ 38 KB
...
| 队列监控 | 每端口队列长度 | 无 |
| 探测包 | 周期性发送 | 无 |
| 重排序缓冲区 | 无 | 实际 < 50 KB/ToR (仿真中无reorder) |
| 重排序缓冲区(最坏) | - | ~125 MB/ToR |

**实际结论**:
- 本次仿真中MixHash **未触发** reorder，内存开销极小(< 50 KB/ToR)
- 即使触发reorder，内存开销也在可控范围内
- 流完成时自动清理机制确保内存及时释放
