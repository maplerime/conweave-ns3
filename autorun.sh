...
TOPOLOGY="fat_k8_100G_400G_OS10" # fat-tree k=8, OS=10, 1280 hosts
...
# ECMP-Conweave mode with different fecmp_bg levels
cecho "GREEN" "\n----------------------------------"
cecho "GREEN" "Run ECMP-Conweave experiments with different fecmp_bg levels..."

# fecmp_bg = 0 (all conweave)
FLOW_FILE="moe_1280group_256to8_8round_8KB.txt"
cecho "YELLOW" "Running: ecmp-conweave fecmp_bg=0 (all conweave), flow: ${FLOW_FILE}"
python3 run.py --lb ecmp-conweave --pfc 1 --irn 1 --simul_time ${RUNTIME} --netload ${NETLOAD} --topo ${TOPOLOGY} --flow_file ${FLOW_FILE} --fecmp_bg 0 2>&1 > /dev/null &
sleep 0.1

# fecmp_bg = 64
FLOW_FILE="moe_1280group_256to8_8round_8KB_hybrid_64fecmp.txt"
cecho "YELLOW" "Running: ecmp-conweave fecmp_bg=64, flow: ${FLOW_FILE}"
python3 run.py --lb ecmp-conweave --pfc 1 --irn 1 --simul_time ${RUNTIME} --netload ${NETLOAD} --topo ${TOPOLOGY} --flow_file ${FLOW_FILE} --fecmp_bg 64 2>&1 > /dev/null &
sleep 0.1

# fecmp_bg = 128
FLOW_FILE="moe_1280group_256to8_8round_8KB_hybrid_128fecmp.txt"
cecho "YELLOW" "Running: ecmp-conweave fecmp_bg=128, flow: ${FLOW_FILE}"
python3 run.py --lb ecmp-conweave --pfc 1 --irn 1 --simul_time ${RUNTIME} --netload ${NETLOAD} --topo ${TOPOLOGY} --flow_file ${FLOW_FILE} --fecmp_bg 128 2>&1 > /dev/null &
sleep 0.1

# fecmp_bg = 192 (all fecmp)
FLOW_FILE="moe_1280group_256to8_8round_8KB_hybrid_192fecmp.txt"
cecho "YELLOW" "Running: ecmp-conweave fecmp_bg=192 (all fecmp), flow: ${FLOW_FILE}"
python3 run.py --lb ecmp-conweave --pfc 1 --irn 1 --simul_time ${RUNTIME} --netload ${NETLOAD} --topo ${TOPOLOGY} --flow_file ${FLOW_FILE} --fecmp_bg 192 2>&1 > /dev/null &
sleep 0.1
