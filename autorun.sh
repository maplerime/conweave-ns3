...
sleep 0.1
python3 run.py --lb hybrid --pfc 1 --irn 0 --simul_time ${RUNTIME} --netload ${NETLOAD} --topo ${TOPOLOGY} --hybrid_threshold 0 --hybrid_ratio 0.2 2>&1 > /dev/null &
sleep 0.1
...
sleep 0.1
python3 run.py --lb hybrid --pfc 0 --irn 1 --simul_time ${RUNTIME} --netload ${NETLOAD} --topo ${TOPOLOGY} --hybrid_threshold 0 --hybrid_ratio 0.2 2>&1 > /dev/null &
sleep 0.1
