#!/usr/bin/env python3
"""
Generate Fat-tree topology for custom configuration:
- 5 pods out of k=8 fat-tree
- 256 hosts per pod (4 ToR × 64 hosts)
- Total 1280 hosts
- Host-to-Tor: round-robin within each pod
"""

k_fat = 8
pods_used = 5  # Use only 5 out of 8 pods
hosts_per_pod = 256
tors_per_pod = 4  # ToR switches per pod
hosts_per_tor = 64  # hosts per ToR (256 / 4)
...
n_core = int(k_fat / 2 * k_fat / 2)  # 16 cores
n_agg_per_pod = int(k_fat / 2)  # 4 agg per pod
n_tor_per_pod = tors_per_pod  # 4 tor per pod
n_agg_total = n_agg_per_pod * pods_used
n_tor_total = n_tor_per_pod * pods_used
n_server_total = hosts_per_pod * pods_used  # 1280
n_core_total = n_core  # 16 cores
...
print("Connection pattern: round-robin (host→Tor: 1→1, 2→2, 3→3, 4→4, 5→1, ...)")
...
filename = "fat_k8_5pods_256perPod_100G_400G_OS1.txt"
...
with open(filename, "w") as f:
    ...
    for pod_id in range(pods_used):
        for tor_id in range(n_tor_per_pod):
            ...
            for h in range(hosts_per_tor):
                ...
                # Round-robin: host h connects to tor (h % 4)
    ...
    # For k=8: agg (pod p, agg a) connects to cores: a*4 to a*4+3
    for pod_id in range(pods_used):
        for agg_id in range(n_agg_per_pod):
            agg_global_id = i_agg + pod_id * n_agg_per_pod + agg_id
