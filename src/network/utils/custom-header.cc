...
void CustomHeader::Serialize (Buffer::Iterator start) const{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  i.WriteHtonU16 (udp.ecmp_counter);  // Write ecmp_counter
		  udp.ih.Serialize(i);
...
CustomHeader::Deserialize (Buffer::Iterator start)
{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  udp.ecmp_counter = i.ReadNtohU16 ();  // Read ecmp_counter
		  if (getInt)
...
uint32_t CustomHeader::GetUdpHeaderSize(void){
	return 8 + sizeof(udp.pg) + sizeof(udp.seq) + sizeof(udp.tag) + sizeof(udp.ecmp_counter) + IntHeader::GetStaticSize();
}
